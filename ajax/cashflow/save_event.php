...
// A card event marked 'paid' is already reflected in the weekly card balance.
$paid   = ($paidby === 'card' && !empty($_POST['paid'])) ? 1 : 0;
...
try {
	...
	if ($id > 0) {
		$db->prepare("UPDATE cash_events SET etype=?, label=?, amount=?, ym=?, week=?, paidby=?, paid=?, updated_at=NOW(), user_id=? WHERE id=?")
		   ->execute([$etype, $label, $amount, $ym, $week, $paidby, $paid, $_SESSION['user_id'] ?? null, $id]);
	} else {
		$db->prepare("INSERT INTO cash_events (etype, label, amount, ym, week, paidby, paid, user_id) VALUES (?,?,?,?,?,?,?,?)")
		   ->execute([$etype, $label, $amount, $ym, $week, $paidby, $paid, $_SESSION['user_id'] ?? null]);
	}
	echo 'ok';
} catch (Throwable $e) {
